<?php /**/ ?><?php
 ...
 $Table="DiveSite";$Columns = 2;
     
      
$conn = mysql_connect($serverhost, $user, $password);
       if($conn){ $msg="OK ".$user." is connected<br>"; OutputMessage($msg);}
echo('Database: '.$db);
...
echo "global \$db, \$user, \$serverhost, \$password;\n";
               ...
               else
                {
                	if($Type[$i]=='date')                	
                	 { 
                	 	 ...
                	 	 echo "if(\$Mode=='EDIT')\n";             	
                	   echo "{echo '<A HREF=\"#\" onClick=\"cal.select(document.forms[\'".$Table.Edit."\'].".$Field[$i].",\'anchor\',\'NNN-dd-yyyy\');return false;\" NAME=\"anchor\" ID=\"anchor\">Calendar</A>';}";
                     echo "\nelse \n";
                 	   echo "{echo '<A HREF=\"#\" onClick=\"cal.select(document.forms[\'".$Table.Entry."\'].".$Field[$i].",\'anchor\',\'NNN-dd-yyyy\');return false;\" NAME=\"anchor\" ID=\"anchor\">Calendar</A>';}";
                     echo("\necho(\"</td>\");\n");
                	   echo("echo stripslashes(\"");
                	}
                	   	else
...
echo "echo \"<tr>\";\n";
for($i=0;$i<=$rows-1;$i++)
 { echo "echo \"<td align='center'>\".\$rowdata[$i].\"&nbsp; </td>\";\n";}
