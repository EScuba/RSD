...
#ValidUserForProgram($_SESSION['User'],'DiveSite.php');
...
#------------------------------Get the Treasure Trove Records ---------------------


$connection = mysql_connect($serverhost,$user,$password) or die('ERROR!!  Cannot connect to MySql');
$rs = mysql_select_db($db,$connection)    or die('ERROR!! Cannot connect to aquatreasurequest database');
$sql="select * from TreasureTrove order by TreasureTroveCode";
$result = mysql_query($sql,$connection) or die("ERROR!! TreasureTrove GetNumRecs failure");
$NumTreasureTroveRecords = mysql_num_rows($result);
mysql_close($connection); 

#----------------- now take the Treasure Trove Records and put them into the DiveSites Database if they do not exist there---------

 for($i=1;$i<=$NumTreasureTroveRecords;$i++)
{
$rowdata=mysql_fetch_row($result);

$DiveSiteStatus='PENDING';
$DiveSiteEnteredBy='TreasureTrove';
$DiveSiteDateEntered=date("y.m.d.");
$DiveSiteProvince=$rowdata[5];
$DiveSiteCountry=$rowdata[6];
$DiveSiteName=$rowdata[7];
$DiveSiteMajorName=$rowdata[8];
$DiveSiteMinorName=$rowdata[9];
$DiveSiteDifficulty=$rowdata[10];
$DiveSiteDepthMin=$rowdata[11];
$DiveSiteExactLat=$rowdata[13];
$DiveSiteExactLong=$rowdata[14];
$DiveSiteShoreLat=$rowdata[15];
$DiveSiteShoreLong=$rowdata[16];

$connection = mysql_connect($serverhost,$user,$password) or die('ERROR!!  Cannot connect to MySql');
$rs = mysql_select_db($db,$connection)    or die('ERROR!! Cannot connect to aquatreasurequest database');
$sql="select * from DiveSite where DiveSiteProvince='".$DiveSiteProvince."' AND DiveSiteCountry='".$DiveSiteCountry."' AND DiveSiteName='".$DiveSiteName."' AND DiveSiteMajorName='".$DiveSiteMajorName."' AND DiveSiteMinorName='".$DiveSiteMinorName."' AND DiveSiteExactLat=".$DiveSiteExactLat." AND DiveSiteExactLong=".$DiveSiteExactLong." AND DiveSiteShoreLat=".$DiveSiteShoreLat." AND DiveSiteShoreLong=".$DiveSiteShoreLong;
echo("<br />sql: ".$sql);
$result2 = mysql_query($sql,$connection);
mysql_close($connection);  
if($result2)
 {$NumDiveSiteRecords = mysql_num_rows($result2);}
 else
  {$NumDiveSiteRecords=0;}
  
echo("<br />recs: ".$NumDiveSiteRecords);  

if($NumDiveSiteRecords==0)
  {
  	echo ('<br />write record');
  	Db_Add();
  }
 else
  {
  	echo('<br />record already on file');
  }
}
 		 	
#----------Now that the divesite files are updated from the TreasureTroves ---- update the divesite pointers ---


$connection = mysql_connect($serverhost,$user,$password) or die('ERROR!!  Cannot connect to MySql');
$rs = mysql_select_db($db,$connection)    or die('ERROR!! Cannot connect to aquatreasurequest database');
$sql="select * from DiveSite order by DiveSiteId";
$result2 = mysql_query($sql,$connection);
mysql_close($connection);  
...
for($i=1;$i<=$NumDiveSiteRecords;$i++)
{
	$rowdata=mysql_fetch_row($result2);
	$DiveSiteId=$rowdata[0];
...
$connection = mysql_connect($serverhost,$user,$password) or die('ERROR!!  Cannot connect to MySql');
$rs = mysql_select_db($db,$connection)    or die('ERROR!! Cannot connect to aquatreasurequest database');
$sql="select * from TreasureTrove where TreasureTroveProvince='".$DiveSiteProvince."' AND TreasureTroveCountry='".$DiveSiteCountry."' AND TreasureTroveSiteName='".$DiveSiteName."' AND TreasureTroveMajorName='".$DiveSiteMajorName."' AND TreasureTroveMinorName='".$DiveSiteMinorName."' AND TreasureTroveExactLat=".$DiveSiteExactLat." AND TreasureTroveExactLong=".$DiveSiteExactLong." AND TreasureTroveShoreLat=".$DiveSiteShoreLat." AND TreasureTroveShoreLong=".$DiveSiteShoreLong;
echo('TreasureTroveFind: '.$sql);

$result3 = mysql_query($sql,$connection);
mysql_close($connection); 
	
if($result3)
 {$NumTreasureTroveRecords = mysql_num_rows($result3);}
 else
  {$NumTreasureTroveRecords=0;}	
  echo('<br />Updating: '.$NumTreasureTroveRecords);
  for($j=1;$j<=$NumTreasureTroveRecords;$j++)
    {
	     $rowdata4=mysql_fetch_row($result3);
	     $TreasureTroveId=$rowdata4[0];
	     $TreasureTroveDiveSiteId=$DiveSiteId;
	     
	 $connection = mysql_connect($serverhost,$user,$password) or die('ERROR!!  Cannot connect to MySql');
   $rs = mysql_select_db($db,$connection)    or die('ERROR!! Cannot connect to aquatreasurequest database');
   $sql="update TreasureTrove SET ";
   $sql=$sql."TreasureTroveDiveSiteId='".strip_tags(addslashes($TreasureTroveDiveSiteId))."' where TreasureTroveId='".$TreasureTroveId."'"; 
   echo('<br />Sql: '.$sql);
   $result4 = mysql_query($sql,$connection) or die("ERROR!! TreasureTrove DATA update failure");   
	 mysql_close($connection);     
